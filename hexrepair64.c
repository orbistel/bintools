...
FileSignature known_signatures[] = {
    ...
    {"PNG", {0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0xA}, 8, 0, "PNG image", "png"},
...
int main(int argc, char *argv[]) {
    ...
    if (verbose || test_mode) {
        int lines_to_show;
        if (byte_count < 1024) {
            lines_to_show = (int)(byte_count / BYTES_PER_LINE);
            if (lines_to_show < 1) lines_to_show = 1;
        } else {
